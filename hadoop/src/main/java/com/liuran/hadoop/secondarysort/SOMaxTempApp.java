...
public class SOMaxTempApp {
    ...
    private static void secondarySort(String[] args) throws Exception{
        ...
        job.setJarByClass(SOMaxTempApp.class);
        job.setJobName("SOMaxTempApp-secondary");

        //设置输入数据源,以及对应的Mapper(单个作业可以采用简单设置方式)
        MultipleInputs.addInputPath(job, new Path(args[0] + "/seq"),
                SequenceFileInputFormat.class, SOMaxTempMapper.class);

        //设置Mapper key-value输出类型
        job.setMapOutputKeyClass(SOMaxTempKeyWritable.class);
...
//                new InputSampler.RandomSampler<SOMaxTempKeyWritable, FloatWritable>(0.2, 2000, spiltSize);
